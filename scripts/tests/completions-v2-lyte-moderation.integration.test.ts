/**
 * LYTE — Completions V2 over-aggressive moderation reproducer.
 *
 * Context:
 *   Bug report 2026-04-13 from @cabernathy (Wellspring) via #support-gloo-ai.
 *   LYTE v2 (uses Gloo Completions V2) refused a benign homestead-exemption /
 ...
 *   without any of LYTE's infrastructure, prompts, or middleware. If the
 *   refusal shows up here, the problem lives in the Gloo AI platform
 *   (Completions V2 safety layer), not in LYTE.
 *
 * Behavior:
...
// The LYTE app does not disclose which routing mode it uses, so we try all
        ...
        register(
          ...
          async () => {
            ...
            if (refusals.length > 0) {
              // eslint-disable-next-line no-console
              console.error(
                `[${routing.label}][${shape.label}] REPRODUCED the LYTE bug ` +
