...
export function ChatPanel({ models }: { models: ModelSummary[] }) {
  ...
  // Bumped when a turn lands so the sidebar picks up the conversation's new
  ...
  async function send(text: string) {
    ...
    try {
      ...
      setBusy(false);
      // After the reply, not before: the summary row is written when the
      // stream ends, so a list fetched any earlier would not yet contain
